...
from gpa_standards import ALL_STANDARD_NAMES, calculate_weighted_gpa
...
with col_extract:
    extract_btn = st.button("🤖 AI 提取", disabled=not uploaded or not api_key, use_container_width=True)
...
if manual_btn:
    st.session_state["courses"] = [
        {"course": "", "credits": 0.0, "score": 0.0, "uncertain": False}
    ]
    st.session_state["extract_done"] = True
...
if st.session_state.get("extract_done"):
    ...
    if has_uncertain:
        st.warning("⚠️ 标黄的行表示 AI 对提取结果不太确定，请重点检查。")
...
if st.session_state.get("edited_df") is not None:
    ...
    if valid_df.empty:
        ...
        courses_for_calc = [
            {"course": row["课程名称"], "credits": row["学分"], "score": row["成绩"]}
        ...
        # ── 单标准模式 ──
        with tab_single:
            standard = st.selectbox("选择绩点计算标准", ALL_STANDARD_NAMES + ["自定义"])

            custom_ranges = None
            if standard == "自定义":
                st.markdown("请定义你的绩点分数段（从高到低）：")
                custom_df = st.data_editor(
                    pd.DataFrame({"最低分": [90, 80, 70, 60, 0], "最高分": [100, 89, 79, 69, 59], "绩点": [4.0, 3.0, 2.0, 1.0, 0.0]}),
                    num_rows="dynamic",
                    use_container_width=True,
            ...
            result = calculate_weighted_gpa(courses_for_calc, standard, custom_ranges)

            result_df = pd.DataFrame(result["courses"])
            result_df.columns = ["课程名称", "学分", "成绩", "绩点"]
            st.dataframe(result_df, use_container_width=True, hide_index=True)

            st.metric("加权平均绩点 (GPA)", f"{result['gpa']:.4f}")
        ...
        with tab_compare:
            ...
            for std_name in ALL_STANDARD_NAMES:
                r = calculate_weighted_gpa(courses_for_calc, std_name)
                compare_data.append({"绩点标准": std_name, "加权平均GPA": round(r["gpa"], 4)})
